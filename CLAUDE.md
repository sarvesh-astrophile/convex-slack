# Open Slack Project Memory

## Project Overview

**Open Slack** is a modern, open-source chat application built with a cutting-edge TypeScript stack. It uses a monorepo architecture managed by Bun workspaces and Turborepo, featuring a TanStack Start frontend and Convex backend.
...
| Layer | Technology |
|-------|-----------|
| Runtime | Bun v1.3.3, TypeScript, Turborepo monorepo |
| Web App | TanStack Start (React 19) + TanStack Router, Vite 7, TailwindCSS v4, shadcn/ui |
| Web State | TanStack Query + Convex React Query, TanStack Form + Zod |
| Web UI | react-resizable-panels, Sonner, Lucide React |
...
│   │   │   ├── auth.config.ts  # Auth config export
...
### Convex Tables (`packages/backend/convex/schema.ts`)

The schema is currently minimal and uses Better-Auth's Convex adapter for authentication tables.

**Better-Auth Tables (auto-managed by adapter):**
- `users` - User accounts
...
| `bun run dev:setup` | Configure Convex (first-time setup) |
| `bun run build` | Build all applications |
| `bun run check` | Run Biome linting and formatting |
| `bun run check-types` | TypeScript type checking |
| `bun run deploy` | Deploy infrastructure via Alchemy |
...
- `VITE_CONVEX_SITE_URL` - Convex site URL for auth callbacks (required)

### Backend (`packages/backend`)
- Better-Auth configuration variables

### Environment Validation
Environment variables are validated using T3 Env in `packages/env/src/web.ts`.

## Key Features

### Authentication
- **Better-Auth Integration:** Full authentication using Better-Auth with Convex plugin
- **Authentication Methods:**
  - Email/password authentication
  - Session-based auth with Convex integration
- **Session Management:** Secure session handling via Better-Auth
- **Protected Routes:** Authentication guards on dashboard and protected pages
...
1. **TanStack Start**: Modern React framework with SSR, file-based routing via TanStack Router
2. **Convex**: Reactive backend with real-time subscriptions
3. **Better-Auth**: Type-safe authentication with multiple providers support
4. **TailwindCSS v4**: Latest version with CSS variables for theming
5. **shadcn/ui**: Headless, accessible UI components
6. **Biome**: Fast linter and formatter (replaces ESLint + Prettier)
...
- **TailwindCSS classes:** Sorted via `useSortedClasses` rule
...
| Package | Purpose |
|---------|---------|
| `@tanstack/react-start` | React framework with SSR |
...
| `@convex-dev/better-auth` | Better-Auth Convex adapter |
| `better-auth` | Authentication library |

### UI Package (`packages/ui`)
| Package | Purpose |
|---------|---------|
| `@base-ui/react` | Headless UI primitives (Base UI) |
| `tailwindcss` | Styling |
...
3. **Convex dashboard:** Access via `npx convex dashboard`

4. **Turborepo tasks:** Configured in `turbo.json` (shared `build`, `check-types`, `dev`)
...
6. **Deployment:**
   ```bash
   bun run deploy  # Deploys via Alchemy to Cloudflare Workers
   ```
